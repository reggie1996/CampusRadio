...
import com.jzone.entity.ContactExt;
import com.jzone.entity.MsgCode;
import com.jzone.entity.ObservableHoriScrollView;
...
public class ChannelFragment extends Fragment implements OnChannelListener,
	...
	private void onSessionPowerClicked(boolean wasOn, boolean act, int idx) {
		...
		if (!wasOn && !channelSelectIsBusy && selfOnline && currSession == null) {
			...
			((TLActivity) uiContext).stopDialogSession(selfId, 0);
		} else {
			if (!selfOnline)
	...
	private void uiEnterChannel(ChannelExt che1) {
		if (che1.cid.getType() != Constant.SESSION_TYPE_TOURLINK) {
			...
			((TLActivity) uiContext).showTourlinkChannelScroller(tlChannels
					.size());
		}
	}
	...
	private void showChannelModifyDialog() {
		...
		if (mChs == null || mChs.size() < 1) {
			...
			((TLActivity) uiContext).showModifyChannelSpinner(mChs);
		}
	}

	private void showChannelDefaultDialog() {
		((TLActivity) uiContext).showSetDefaultSpinner();
	}

	private void showChannelDeleteDialog() {
		((TLActivity) uiContext)
	...
	private void syncCurrentSessionDelayed(int delayMs) {
		uiHandler.postDelayed(new Runnable() {
			...
			public void run() {
				if (sessionApi != null) {
					((TLActivity) uiContext).showProgressCircle();
	...
	public void onSessionGet(int selfUserId, int type, int sessionId, int initiator) {
		((TLActivity) uiContext).dismessProgressCircle();
	...
	private void funcGetAdverChannels() {
		if (selfOnline && myChannels == null) {
			if (!simpleChannelMode) {
				((TLActivity) uiContext).showProcessing(getResources().getString(R.string.getting_pub_channel));
				if (channelApi != null)
	...
	public void onAdverChannelsGet(int userId, Channel dfltChannel,
		...
		((TLActivity) uiContext).dismissProcessing();
	...
	private void funcGetChannelList() {
		if (selfOnline) {
			((TLActivity) uiContext).showProcessing(getResources().getString(R.string.getting_channels));
			if (channelApi != null)
	...
	private void mergeChannelList(Channel dfltCh, List<Channel> chs) {
		...
		List<Integer> myADCs = ((TLActivity)uiContext).getMyPreferedChannels();
	...
	public void onPubChannelSearchResult(int uid,
		...
		if (uid > 0 && foundChs.size() > 0) {
			...
			if (diffChs.size() > 0)
				((TLActivity) uiContext).showFocusChannelSpinner(diffChs);
			else
